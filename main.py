...
def load_saved_theme():
    try:
        theme_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "theme_preference.txt")
...
class SSSnakeExecutor(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SSSnake Executor - WRISTONCARTIER Edition")
    ...
    def try_load_dll(self) -> bool:
        """Try to load the DLL from the lib directory"""
        try:
            ...
            # Check for MacSploit-based injector first (preferred method)
            macsploit_injector_path = os.path.join(lib_dir, "macsploit_injector.py")
            if os.path.exists(macsploit_injector_path):
                self.log_to_console(f"[INFO] Found MacSploit-based injector at {macsploit_injector_path}")
                try:
                    ...
                    spec = importlib.util.spec_from_file_location("macsploit_injector", macsploit_injector_path)
                    injector_module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(injector_module)
                    
                    # Create a wrapper class for the MacSploit injector with the same interface as our DLL wrapper
                    class MacSploitInjector:
                        def __init__(self, module):
                            self.module = module
                            self.log_to_console(f"[INFO] Initialized SSSnake MacSploit injector")
                        ...
                        def inject_into_process(self, target_pid=None):
                            try:
                                # If we have a target PID, set it
                                if target_pid:
                                    self.module.set_target_process(target_pid)
                                
                                return self.module.inject_into_process()
                            except Exception as e:
                        ...
                        def cleanup_injector(self):
                            try:
                                self.module.cleanup_injector()
                                return True
                            except Exception as e:
                                print(f"Cleanup error: {e}")
                                return False
                    
                    # Use the MacSploit injector
                    self.dll = MacSploitInjector(injector_module)
                    self.log_to_console(f"[INFO] Successfully loaded MacSploit injector")
                    self.simulation_mode = False
                    self.dll_loaded = True
                    return True
                except Exception as e:
                    self.log_to_console(f"[WARNING] Failed to initialize MacSploit injector: {e}")
                    self.log_to_console("[INFO] Falling back to Python injector or simulation mode")
            
            # Check for Python-based injector as fallback
            py_injector_path = os.path.join(lib_dir, "roblox_injector.py")
            if os.path.exists(py_injector_path):
                self.log_to_console(f"[INFO] Found Python-based injector at {py_injector_path}")
                try:
                    # Add lib directory to path if not already there
                    if lib_dir not in sys.path:
                        sys.path.append(lib_dir)
                    ...
                    # Import the injector module
                    import importlib.util
                    spec = importlib.util.spec_from_file_location("roblox_injector", py_injector_path)
                    injector_module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(injector_module)
                    
                    # Create a wrapper class for the Python injector with the same interface as our DLL wrapper
                    class PythonInjector:
                        def __init__(self, module):
                            self.module = module
                            self.log_to_console(f"[INFO] Initialized SSSnake Python injector")
                    ...
                    # Use the Python injector
                    self.dll = PythonInjector(injector_module)
                    self.log_to_console(f"[INFO] Successfully loaded Python injector")
                    self.simulation_mode = False
                    self.dll_loaded = True
                    return True
                except Exception as e:
                    self.log_to_console(f"[WARNING] Failed to initialize Python injector: {e}")
                    self.log_to_console("[INFO] Falling back to DLL or simulation mode")
            
            # Determine the correct DLL path based on platform (fallback)
            ...
            if not os.path.exists(dll_path):
                self.log_to_console(f"[INFO] Native injector library not found at {dll_path}")
            ...
            self.log_to_console(f"[INFO] Attempting to load native injector from {dll_path}")
            ...
            try:
                ...
                self.log_to_console(f"[INFO] Successfully loaded native injector from {dll_path}")
                self.simulation_mode = False
                self.dll_loaded = True
                return True
            except Exception as e:
    ...
    def inject(self):
        ...
        if roblox_running:
            ...
            if self.simulation_mode:
                ...
                # Simulate instant injection success
                self._simulate_injection_success()
            else:
                # Use the real injector with proper error handling
                self.log_to_console("[INFO] Injecting into Roblox...")
                
                try:
                    # Call injection directly - this now uses a proper thread internally
                    result = self.dll.inject_into_process(roblox_pid)
                    ...
                    self.log_to_console(f"[ERROR] Injection error: {str(e)}")
                    self.status_label.setText(" ● Injection Failed")
                    self.status_label.setStyleSheet("color: #ff0000;")
    ...
    def execute_script(self):
        ...
        if " ● Injected" in self.status_label.text():
            ...
            # Use the real executor with proper error handling
            self.log_to_console("[INFO] Executing script in Roblox...")
            
            try:
                # Execute the script directly using our real injector
                result = self.dll.execute_script(script)
                ...
                self.log_to_console(f"[ERROR] Script execution error: {str(e)}")
    ...
    def _execution_success(self):
        """Handle a successful script execution"""
        self.log_to_console("[SUCCESS] Script executed successfully!")
    ...
    def update_theme(self):
        ...
        try:
            ...
            try:
                with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "theme_preference.txt"), "w") as f:
...
"""
    ...
    def show_about_dialog(self):
        ...
        about_text = """
        <h2>SSSnake Executor - WRISTONCARTIER Edition</h2>
        <p>Version 1.0.0</p>
        <p>A professional Roblox Lua script executor for macOS.</p>
        <p> 2025 WRISTONCARTIER. All rights reserved.</p>
        <p>This software is for educational purposes only.</p>
        """
        
        QMessageBox.about(self, "About SSSnake", about_text)
